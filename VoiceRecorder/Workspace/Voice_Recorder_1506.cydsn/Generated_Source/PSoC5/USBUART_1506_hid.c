...
volatile uint8 USBUART_1506_hidProtocol[USBUART_1506_MAX_INTERFACES_NUMBER];
 ...
 * reload timer.*/
volatile uint8 USBUART_1506_hidIdleRate[USBUART_1506_MAX_INTERFACES_NUMBER];

/** This variable contains the timer counter, which is decremented and reloaded 
 * by the USBFS_UpdateHIDTimer() API.*/
volatile uint8 USBUART_1506_hidIdleTimer[USBUART_1506_MAX_INTERFACES_NUMBER]; /* HID device idle rate value */
...
/*******************************************************************************
* Function Name: USBUART_1506_UpdateHIDTimer
****************************************************************************//**
*
...
*
*******************************************************************************/
uint8 USBUART_1506_UpdateHIDTimer(uint8 interface) 
{
    uint8 stat = USBUART_1506_IDLE_TIMER_INDEFINITE;

    if(USBUART_1506_hidIdleRate[interface] != 0u)
    {
        if(USBUART_1506_hidIdleTimer[interface] > 0u)
        {
            USBUART_1506_hidIdleTimer[interface]--;
            stat = USBUART_1506_IDLE_TIMER_RUNNING;
        }
        else
        {
            USBUART_1506_hidIdleTimer[interface] = USBUART_1506_hidIdleRate[interface];
            stat = USBUART_1506_IDLE_TIMER_EXPIRED;
        }
    }
...
/*******************************************************************************
* Function Name: USBUART_1506_GetProtocol
****************************************************************************//**
*
...
*
*******************************************************************************/
uint8 USBUART_1506_GetProtocol(uint8 interface) 
{
    return(USBUART_1506_hidProtocol[interface]);
}


/*******************************************************************************
* Function Name: USBUART_1506_DispatchHIDClassRqst
****************************************************************************//**
*
...
uint8 USBUART_1506_DispatchHIDClassRqst(void) 
{
    ...
    uint8 interfaceNumber = (uint8) USBUART_1506_wIndexLoReg;
    ...
    if (0u != (USBUART_1506_bmRequestTypeReg & USBUART_1506_RQST_DIR_D2H))
    {
        ...
        switch (USBUART_1506_bRequestReg)
        {
            case USBUART_1506_GET_DESCRIPTOR:
                if (USBUART_1506_wValueHiReg == USBUART_1506_DESCR_HID_CLASS)
                {
                    USBUART_1506_FindHidClassDecriptor();
                    if (USBUART_1506_currentTD.count != 0u)
                    {
                        requestHandled = USBUART_1506_InitControlRead();
                    }
                }
                else if (USBUART_1506_wValueHiReg == USBUART_1506_DESCR_HID_REPORT)
                {
                    USBUART_1506_FindReportDescriptor();
                    if (USBUART_1506_currentTD.count != 0u)
                    {
                        requestHandled = USBUART_1506_InitControlRead();
                    }
            ...
            case USBUART_1506_HID_GET_REPORT:
                USBUART_1506_FindReport();
                if (USBUART_1506_currentTD.count != 0u)
                {
                    requestHandled = USBUART_1506_InitControlRead();
                }
            ...
            case USBUART_1506_HID_GET_IDLE:
                ...
                if ((interfaceNumber < USBUART_1506_MAX_INTERFACES_NUMBER) && (USBUART_1506_wValueLoReg == 0u)) 
                {
                    USBUART_1506_currentTD.count = 1u;
                    USBUART_1506_currentTD.pData = &USBUART_1506_hidIdleRate[interfaceNumber];
                    requestHandled  = USBUART_1506_InitControlRead();
                }
                break;
                
            case USBUART_1506_HID_GET_PROTOCOL:
                /* Validate interfaceNumber */
                if( interfaceNumber < USBUART_1506_MAX_INTERFACES_NUMBER)
                {
                    USBUART_1506_currentTD.count = 1u;
                    USBUART_1506_currentTD.pData = &USBUART_1506_hidProtocol[interfaceNumber];
                    requestHandled  = USBUART_1506_InitControlRead();
                }
                break;
    ...
    {   
        ...
        switch (USBUART_1506_bRequestReg)
        {
            case USBUART_1506_HID_SET_REPORT:
                USBUART_1506_FindReport();
                if (USBUART_1506_currentTD.count != 0u)
                {
            ...
            case USBUART_1506_HID_SET_IDLE:
                /* This function does not support multiple reports per interface */
                /* Validate interfaceNumber and Report ID (should be 0): Do not support Idle per Report ID */
                if ((interfaceNumber < USBUART_1506_MAX_INTERFACES_NUMBER) && (USBUART_1506_wValueLoReg == 0u))
                {
                    USBUART_1506_hidIdleRate[interfaceNumber] = (uint8)USBUART_1506_wValueHiReg;
                    ...
                    */
                    if(USBUART_1506_hidIdleRate[interfaceNumber] <
                       USBUART_1506_hidIdleTimer[interfaceNumber])
                    {
                        /* Set the timer to zero and let the UpdateHIDTimer() API return IDLE_TIMER_EXPIRED status*/
                        USBUART_1506_hidIdleTimer[interfaceNumber] = 0u;
                    }
                    ...
                    */
                    else if(USBUART_1506_hidIdleTimer[interfaceNumber] <= 1u)
                    {
                        /* Do nothing.
                    ...
                    {   /* Reload the timer*/
                        USBUART_1506_hidIdleTimer[interfaceNumber] =
                        USBUART_1506_hidIdleRate[interfaceNumber];
                    }
                    requestHandled = USBUART_1506_InitNoDataControlTransfer();
                }
                break;

            case USBUART_1506_HID_SET_PROTOCOL:
                /* Validate interfaceNumber and protocol (must be 0 or 1) */
                if ((interfaceNumber < USBUART_1506_MAX_INTERFACES_NUMBER) && (USBUART_1506_wValueLoReg <= 1u))
                {
                    USBUART_1506_hidProtocol[interfaceNumber] = (uint8)USBUART_1506_wValueLoReg;
                    requestHandled = USBUART_1506_InitNoDataControlTransfer();
                }
                break;
...
void USBUART_1506_FindHidClassDecriptor(void) 
{
    ...
    interfaceN = (uint8) USBUART_1506_wIndexLoReg;
    ...
    pTmp = &pTmp[USBUART_1506_interfaceSetting[interfaceN]];
    ...
    USBUART_1506_currentTD.count = *pDescr;
    USBUART_1506_currentTD.pData = pDescr;
}
...
void USBUART_1506_FindReportDescriptor(void) 
{
    ...
    USBUART_1506_currentTD.count =  ((uint16)((uint16) pDescr[1u] << 8u) | pDescr[0u]);
    USBUART_1506_currentTD.pData = &pDescr[2u];
}


/*******************************************************************************
* Function Name: USBUART_1506_FindReport
****************************************************************************//**
*
...
void USBUART_1506_FindReport(void) 
{
...
#ifdef USBUART_1506_FIND_REPORT_CALLBACK
    USBUART_1506_FindReport_Callback();
#endif /* (USBUART_1506_FIND_REPORT_CALLBACK) */
    
    USBUART_1506_currentTD.count = 0u;   /* Init not supported condition */
    pTmp = USBUART_1506_GetConfigTablePtr(USBUART_1506_configuration - 1u);
    reportType = (uint8) USBUART_1506_wValueHiReg;
    interfaceN = (uint8) USBUART_1506_wIndexLoReg;
    ...
    /* USB_DEVICEx_CONFIGURATIONy_INTERFACEz_TABLE */
    pTmp = (const T_USBUART_1506_LUT CYCODE *) pTmp->p_list;
    if (interfaceN < USBUART_1506_MAX_INTERFACES_NUMBER)
    {
        /* Now use Alternate setting number */
        pTmp = &pTmp[USBUART_1506_interfaceSetting[interfaceN]];
        ...
        if ((reportType >= USBUART_1506_HID_GET_REPORT_INPUT) &&
            (reportType <= USBUART_1506_HID_GET_REPORT_FEATURE))
        {
            /* Get the entry proper TD (IN, OUT or Feature Report Table)*/
            pTmp = &pTmp[reportType - 1u];
            
            /* Get reportID */
            reportType = (uint8) USBUART_1506_wValueLoReg;
            
            /* Validate table support by the HID descriptor, compare table count with reportID */
            if (pTmp->c >= reportType)
            {
                pTD = (T_USBUART_1506_TD *) pTmp->p_list;
                pTD = &pTD[reportType];                          /* select entry depend on report ID*/
                USBUART_1506_currentTD.pData = pTD->pData;   /* Buffer pointer */
                USBUART_1506_currentTD.count = pTD->count;   /* Buffer Size */
                USBUART_1506_currentTD.pStatusBlock = pTD->pStatusBlock;
            }
        }
    }
}
