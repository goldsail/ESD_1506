...
static uint8  USBUART_1506_started = 0u;


/*******************************************************************************
* Function Name: USBUART_1506_CyBtldrCommStart
****************************************************************************//**
*
...
void USBUART_1506_CyBtldrCommStart(void) 
{
    ...
    USBUART_1506_Start(0u, USBUART_1506_DWR_POWER_OPERATION);

    /* USB component started, the correct enumeration will be checked in the first Read operation. */
    USBUART_1506_started = 1u;
}


/*******************************************************************************
* Function Name: USBUART_1506_CyBtldrCommStop.
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: USBUART_1506_CyBtldrCommReset.
****************************************************************************//**
*
...
{
    USBUART_1506_EnableOutEP(USBUART_1506_BTLDR_OUT_EP); 
}


/*******************************************************************************
* Function Name: USBUART_1506_CyBtldrCommWrite.
****************************************************************************//**
*
                                                            ...
                                                            
{
    ...
    USBUART_1506_LoadInEP(USBUART_1506_BTLDR_IN_EP, pData, USBUART_1506_BTLDR_SIZEOF_READ_BUFFER);

    /* Wait unitl host reads data from IN endpoint. */
    while ((USBUART_1506_GetEPState(USBUART_1506_BTLDR_IN_EP) == USBUART_1506_IN_BUFFER_FULL) &&
           (0u != timeoutMs))
    {
        CyDelay(USBUART_1506_BTLDR_WAIT_1_MS);
        timeoutMs--;
    }

    if (USBUART_1506_GetEPState(USBUART_1506_BTLDR_IN_EP) == USBUART_1506_IN_BUFFER_FULL)
    {
...
/*******************************************************************************
* Function Name: USBUART_1506_CyBtldrCommRead.
****************************************************************************//**
*
                                                            ...
                                                            
{
    ...
    if (size > USBUART_1506_BTLDR_SIZEOF_WRITE_BUFFER)
    {
        size = USBUART_1506_BTLDR_SIZEOF_WRITE_BUFFER;
    }

    /* Wait for enumeration first time. */
    if (0u != USBUART_1506_started)
    {
        /* Wait for device enumeration. */
        while ((0u == USBUART_1506_GetConfiguration()) && (0u != timeoutMs))
        {
            CyDelay(USBUART_1506_BTLDR_WAIT_1_MS);
            timeoutMs--;
        }
        ...
        if (0u != USBUART_1506_GetConfiguration())
        {
            (void) USBUART_1506_IsConfigurationChanged();  /* Clear configuration changes state status. */
            USBUART_1506_CyBtldrCommReset();
            
            USBUART_1506_started = 0u;
        }
    }
    else /* Check for configuration changes, has been done by Host. */
    {
        if (0u != USBUART_1506_IsConfigurationChanged()) /* Host could send double SET_INTERFACE request or RESET. */
        {
            if (0u != USBUART_1506_GetConfiguration())   /* Init OUT endpoints when device reconfigured. */
            {
                USBUART_1506_CyBtldrCommReset();
            }
        }
    ...
    while ((USBUART_1506_GetEPState(USBUART_1506_BTLDR_OUT_EP) != USBUART_1506_OUT_BUFFER_FULL) && \
           (0u != timeoutMs))
    {
        CyDelay(USBUART_1506_BTLDR_WAIT_1_MS);
        timeoutMs--;
    ...
    if (USBUART_1506_GetEPState(USBUART_1506_BTLDR_OUT_EP) == USBUART_1506_OUT_BUFFER_FULL)
    {
        *count = USBUART_1506_ReadOutEP(USBUART_1506_BTLDR_OUT_EP, pData, size);
        retCode = CYRET_SUCCESS;
    }
    else
    {
