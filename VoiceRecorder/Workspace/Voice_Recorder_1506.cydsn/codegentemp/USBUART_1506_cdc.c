 ...
 * USBUART_1506_GetDTERate(), USBUART_1506_GetCharFormat(),
 * USBUART_1506_GetParityType(), and USBUART_1506_GetDataBits() APIs.
...
volatile uint8 USBUART_1506_linesCoding[USBUART_1506_MAX_MULTI_COM_NUM][USBUART_1506_LINE_CODING_SIZE] =
{
    /*COM Port 1*/
    {
...
/**Used as a flag for the USBUART_1506_IsLineChanged() API, to inform it that the
 ...
 * support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USBUART_1506_linesChanged[USBUART_1506_MAX_MULTI_COM_NUM];
 ...
 * USBUART_1506_GetLineControl() API. It is an array of 2 elements for COM 
 * port 1 and COM port 2 for MultiCOM port support. In case of 1 COM port, data 
 * is in 0 element.*/
volatile uint16 USBUART_1506_linesControlBitmap[USBUART_1506_MAX_MULTI_COM_NUM];
/** Contains the 16-bit serial state value that was sent using the 
 * \ref USBUART_1506_SendSerialState() API. . It is an array of 2 elements 
 * for COM port 1 and COM port 2 for MultiCOM port support. In case of 1 COM 
 * port, data is in 0 element.*/
volatile uint16 USBUART_1506_serialStateBitmap[USBUART_1506_MAX_MULTI_COM_NUM];
...
volatile uint8  USBUART_1506_cdcDataInEp[USBUART_1506_MAX_MULTI_COM_NUM];
 ...
 * port 2 for MultiCOM port support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USBUART_1506_cdcDataOutEp[USBUART_1506_MAX_MULTI_COM_NUM];
...
volatile uint8  USBUART_1506_cdcCommInInterruptEp[USBUART_1506_MAX_MULTI_COM_NUM];

/*PRIVATE*/

#define USBUART_1506_CDC_IN_EP      (0u)
#define USBUART_1506_CDC_OUT_EP     (1u)
#define USBUART_1506_CDC_NOTE_EP    (2u)

#define USBUART_1506_CDC_EP_MASK    (0x01u)

#define USBUART_1506_GET_EP_COM_NUM(cdcComNums, epType) (((cdcComNums) >> (epType)) & USBUART_1506_CDC_EP_MASK)
...
***************************************/
    ...
    static uint16 USBUART_1506_StrLen(const char8 string[]) ;
    static t_USBUART_1506_cdc_notification USBUART_1506_serialStateNotification =
    {

        USBUART_1506_SERIAL_STATE_REQUEST_TYPE, /* bRequestType    */
        USBUART_1506_SERIAL_STATE,              /* bNotification   */
        0u,                                         /* wValue          */
        0u,                                         /* wValueMSB       */
        0u,                                         /* wIndex          */
        0u,                                         /* wIndexMSB       */
        USBUART_1506_SERIAL_STATE_LENGTH,       /* wLength         */
        0u,                                         /* wLengthMSB      */
        0u,                                         /* wSerialState    */
        0u,                                         /* wSerialStateMSB */
    };
    static uint8 USBUART_1506_activeCom = 0u;
#endif /* (USBUART_1506_ENABLE_CDC_CLASS_API != 0u) */
...
/*******************************************************************************
* Function Name: USBUART_1506_DispatchCDCClassRqst
****************************************************************************//**
*
...
*
* \globalvars
*   USBUART_1506_linesCoding: Contains the current line coding structure.
...
*   USBUART_1506_linesControlBitmap: Contains the current control signal
*     bitmap. It is set by the Host using SET_CONTROL_LINE request and returned
*     to the user code by the USBFS_GetLineControl() API.
*   USBUART_1506_linesChanged: This variable is used as a flag for the
...
uint8 USBUART_1506_DispatchCDCClassRqst(void) 
{
    ...
    comPort = USBUART_1506_GetInterfaceComPort((uint8)USBUART_1506_wIndexLoReg);
    ...
    if (0u != (USBUART_1506_bmRequestTypeReg & USBUART_1506_RQST_DIR_D2H))
    {
        ...
        {
            case USBUART_1506_CDC_GET_LINE_CODING:
                USBUART_1506_currentTD.count = USBUART_1506_LINE_CODING_SIZE;
                USBUART_1506_currentTD.pData = USBUART_1506_linesCoding[comPort];
            ...
            #ifdef USBUART_1506_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK
                requestHandled = USBUART_1506_DispatchCDCClass_CDC_READ_REQUESTS_Callback();
            #endif /* (USBUART_1506_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK) */
                break;
        }
    }
    else
    {
        ...
        switch (USBUART_1506_bRequestReg)
        {
            case USBUART_1506_CDC_SET_LINE_CODING:
                USBUART_1506_currentTD.count  = USBUART_1506_LINE_CODING_SIZE;
                USBUART_1506_currentTD.pData  = USBUART_1506_linesCoding[comPort];
                USBUART_1506_linesChanged[comPort] |= USBUART_1506_LINE_CODING_CHANGED;

                requestHandled = USBUART_1506_InitControlWrite();
                break;

            case USBUART_1506_CDC_SET_CONTROL_LINE_STATE:
                USBUART_1506_linesControlBitmap[comPort] = (uint8) USBUART_1506_wValueLoReg;
                USBUART_1506_linesChanged[comPort]      |= USBUART_1506_LINE_CONTROL_CHANGED;
            ...
            default:
                /* Do not handle this request unless callback is defined. */
            #ifdef USBUART_1506_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK
                requestHandled = USBUART_1506_DispatchCDCClass_CDC_WRITE_REQUESTS_Callback();
            #endif /* (USBUART_1506_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK) */
                break;
        }
    }
...
/***************************************************************************
* Function Name: USBUART_1506_GetInterfaceComPort
************************************************************************//**
*   \internal
...
uint8 USBUART_1506_GetInterfaceComPort(uint8 interface) 
{
    ...
    while (comPort < USBUART_1506_MAX_MULTI_COM_NUM)
    {
        notEp = USBUART_1506_cdcCommInInterruptEp[comPort];

        if (USBUART_1506_EP[notEp].interface == interface)
        {
            res = comPort;
            comPort = USBUART_1506_MAX_MULTI_COM_NUM;
        }
...
#if (USBUART_1506_ENABLE_CDC_CLASS_API != 0u)
/***************************************************************************
* Function Name: USBUART_1506_CDC_Init
************************************************************************//**
*
...
*   USBUART_1506_SUCCESS   CDC interface was initialized correctly
*   USBUART_1506_FAILURE   CDC interface was not initialized
*
* \globalvars
*   USBUART_1506_linesChanged: Initialized to zero.
*   USBUART_1506_cdcDataOutEp: Used as an OUT endpoint number.
*
* \reentrant
*  No.
*
...
uint8 USBUART_1506_CDC_Init(void) 
{
    uint8 comPort;
    uint8 outEp;
    uint8 ret = USBUART_1506_SUCCESS;

    USBUART_1506_activeCom = 0u;
    USBUART_1506_linesChanged[USBUART_1506_COM_PORT1] = 0u;
    USBUART_1506_linesChanged[USBUART_1506_COM_PORT2] = 0u;

    for(comPort = 0u; comPort<USBUART_1506_MAX_MULTI_COM_NUM; comPort++)
    {
        outEp = USBUART_1506_cdcDataOutEp[comPort];
        if((0u != outEp) && (USBUART_1506_MAX_EP > outEp))
        {
            USBUART_1506_EnableOutEP(outEp);
        }

    }

    /* COM Port 1 should be correct to proceed. */
    if ((0u == USBUART_1506_cdcDataInEp[USBUART_1506_COM_PORT1]) \
            || (0u == USBUART_1506_cdcDataOutEp[USBUART_1506_COM_PORT1]) \
            || (0u ==  USBUART_1506_cdcCommInInterruptEp[USBUART_1506_COM_PORT1])
            || (USBUART_1506_cdcDataInEp[USBUART_1506_COM_PORT1] >= USBUART_1506_MAX_EP)
            || (USBUART_1506_cdcDataOutEp[USBUART_1506_COM_PORT1] >= USBUART_1506_MAX_EP)
            || (USBUART_1506_cdcCommInInterruptEp[USBUART_1506_COM_PORT1] >= USBUART_1506_MAX_EP))
    {
        ret = USBUART_1506_FAILURE;
    }
...
/*******************************************************************************
* Function Name: USBUART_1506_PutData
****************************************************************************//**
*
*  This function sends a specified number of bytes from the location specified
*  by a pointer to the PC. The USBUART_1506_CDCIsReady() function should be
...
*  identified. To send zero-length packet, use USBUART_1506_PutData() API
...
* \globalvars
*
*   USBUART_1506_cdcDataInEp: CDC IN endpoint number used for sending
*     data.
*
* \reentrant
*  No.
...
{
    uint8 epNumber = USBUART_1506_cdcDataInEp[USBUART_1506_activeCom];

    /* Limit length to maximum packet size for endpoint. */
    if (length > USBUART_1506_EP[epNumber].bufferSize)
    {
        /* Caution: Data will be lost if length is greater than Max Packet Size. */
        length = USBUART_1506_EP[epNumber].bufferSize;

        /* Halt CPU in debug mode */
        CYASSERT(0u != 0u);
    }

    USBUART_1506_LoadInEP(epNumber, pData, length);
}


/*******************************************************************************
* Function Name: USBUART_1506_StrLen
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: USBUART_1506_PutString
************************************************************************//**
*
...
*  The USBUART_1506_CDCIsReady() function should be called before
*  sending data with a new call to USBUART_1506_PutString(), to be sure
...
void USBUART_1506_PutString(const char8 string[]) 
{
    ...
    strLength = USBUART_1506_StrLen(string);

    do
    {
        /* Limit length to maximum packet size of endpoint. */
        sendLength = (strLength > USBUART_1506_EP[epNumber].bufferSize) ?
                                  USBUART_1506_EP[epNumber].bufferSize : strLength;

        /* Load IN endpoint and expose it to host. */
        USBUART_1506_LoadInEP(epNumber, (const uint8 *)&string[bufIndex], sendLength);
        strLength -= sendLength;

        /* If more data are present to send or full packet was sent */
        if ((strLength > 0u) || (sendLength == USBUART_1506_EP[epNumber].bufferSize))
        {
            bufIndex += sendLength;

            /* Wait until host read data from IN endpoint buffer. */
            while (USBUART_1506_IN_BUFFER_FULL == USBUART_1506_EP[epNumber].apiEpState)
            {
            }
            ...
            */
            if (0u == strLength)
            {
                USBUART_1506_LoadInEP(epNumber, NULL, 0u);
            }
        }
    }
    while (strLength > 0u);
...
/***************************************************************************
* Function Name: USBUART_1506_PutChar
************************************************************************//**
*
...
void USBUART_1506_PutChar(char8 txDataByte) 
{
    ...
    USBUART_1506_LoadInEP(USBUART_1506_cdcDataInEp[USBUART_1506_activeCom], &dataByte, 1u);
}


/*******************************************************************************
* Function Name: USBUART_1506_PutCRLF
****************************************************************************//**
*
...
void USBUART_1506_PutCRLF(void) 
{
    ...
    USBUART_1506_LoadInEP(USBUART_1506_cdcDataInEp[USBUART_1506_activeCom], (const uint8 *)txData, 2u);
}


/*******************************************************************************
* Function Name: USBUART_1506_GetCount
****************************************************************************//**
*
*  This function returns the number of bytes that were received from the PC.
*  The returned length value should be passed to USBUART_1506_GetData() as
*  a parameter to read all received data. If all of the received data is not
*  read at one time by the USBUART_1506_GetData() API, the unread data will
*  be lost.
*
* \return
...
*
* \globalvars
*   USBUART_1506_cdcDataOutEp: CDC OUT endpoint number used.
*
*******************************************************************************/
uint16 USBUART_1506_GetCount(void) 
{
    ...
    uint8  epNumber = USBUART_1506_cdcDataOutEp[USBUART_1506_activeCom];

    if (USBUART_1506_OUT_BUFFER_FULL == USBUART_1506_EP[epNumber].apiEpState)
    {
        bytesCount = USBUART_1506_GetEPCount(epNumber);
    }
    else
    {
        bytesCount = 0u;
...
/*******************************************************************************
* Function Name: USBUART_1506_DataIsReady
****************************************************************************//**
*
*  This function returns a non-zero value if the component received data or
*  received zero-length packet. The USBUART_1506_GetAll() or
*  USBUART_1506_GetData() API should be called to read data from the buffer
...
{
    return (USBUART_1506_GetEPState(USBUART_1506_cdcDataOutEp[USBUART_1506_activeCom]));
}


/*******************************************************************************
* Function Name: USBUART_1506_CDCIsReady
****************************************************************************//**
*
...
*  USBUART_1506_PutData(),USBUART_1506_PutString(),
*  USBUART_1506_PutChar or USBUART_1506_PutCRLF(),
...
*
* \globalvars
*   USBUART_1506_cdcDataInEp: CDC IN endpoint number used.
*
*******************************************************************************/
uint8 USBUART_1506_CDCIsReady(void) 
{
    return (USBUART_1506_GetEPState(USBUART_1506_cdcDataInEp[USBUART_1506_activeCom]));
}


/***************************************************************************
* Function Name: USBUART_1506_GetData
************************************************************************//**
*
...
*  The USBUART_1506_DataIsReady() API should be called first, to be sure
*  that data is received from the host. If all received data will not be read at
*  once, the unread data will be lost. The USBUART_1506_GetData() API should
...
uint16 USBUART_1506_GetData(uint8* pData, uint16 length) 
{
    ...
    length = USBUART_1506_ReadOutEP(epNumber, pData, length);

#if (USBUART_1506_EP_MANAGEMENT_DMA_MANUAL)
    /* Wait until DMA complete transferring data from OUT endpoint buffer. */
    while (USBUART_1506_OUT_BUFFER_FULL == USBUART_1506_GetEPState(epNumber))
    {
    }
...
*
* \globalvars
*   - \ref USBUART_1506_cdcDataOutEp: CDC OUT endpoint number used.
*   - \ref USBUART_1506_EP[].bufferSize: EP max packet size is used as a
...
uint16 USBUART_1506_GetAll(uint8* pData) 
{
    ...
    /* Read data from OUT endpoint buffer. */
    dataLength = USBUART_1506_ReadOutEP(epNumber, pData, USBUART_1506_EP[epNumber].bufferSize);
...
/***************************************************************************
* Function Name: USBUART_1506_GetChar
************************************************************************//**
*
...
uint8 USBUART_1506_GetChar(void) 
{
    ...
    (void) USBUART_1506_ReadOutEP(epNumber, &rxData, 1u);
...
/*******************************************************************************
* Function Name: USBUART_1506_IsLineChanged
****************************************************************************//**
*
...
*  device. The USBUART_1506_GetDTERate(), USBUART_1506_GetCharFormat()
*  or USBUART_1506_GetParityType() or USBUART_1506_GetDataBits() API
*  should be called to read data coding information.
*  The USBUART_1506_GetLineControl() API should be called to read line
*  control information.
*
* \return
...
*
* \globalvars
*  - \ref USBUART_1506_transferState: it is checked to be sure then OUT
*    data phase has been complete, and data written to the lineCoding or
*    Control Bitmap buffer.
*  - \ref USBUART_1506_linesChanged: used as a flag to be aware that
...
uint8 USBUART_1506_IsLineChanged(void) 
{
    ...
    if (USBUART_1506_transferState == USBUART_1506_TRANS_STATE_IDLE)
    {
        if (USBUART_1506_linesChanged[USBUART_1506_activeCom] != 0u)
        {
            state = USBUART_1506_linesChanged[USBUART_1506_activeCom];
            USBUART_1506_linesChanged[USBUART_1506_activeCom] = 0u;
        }
    }
...
/***************************************************************************
* Function Name: USBUART_1506_GetDTERate
************************************************************************//**
*
...
*
* \globalvars
*  USBUART_1506_linesCoding: First four bytes converted to uint32
...
uint32 USBUART_1506_GetDTERate(void) 
{
    ...
    rate = USBUART_1506_linesCoding[USBUART_1506_activeCom][USBUART_1506_LINE_CODING_RATE + 3u];
    rate = (rate << 8u) | USBUART_1506_linesCoding[USBUART_1506_activeCom][USBUART_1506_LINE_CODING_RATE + 2u];
    rate = (rate << 8u) | USBUART_1506_linesCoding[USBUART_1506_activeCom][USBUART_1506_LINE_CODING_RATE + 1u];
    rate = (rate << 8u) | USBUART_1506_linesCoding[USBUART_1506_activeCom][USBUART_1506_LINE_CODING_RATE];
...
/*******************************************************************************
* Function Name: USBUART_1506_GetCharFormat
****************************************************************************//**
*
...
*
* \globalvars
*  USBUART_1506_linesCoding: used to get a parameter.
*
*******************************************************************************/
uint8 USBUART_1506_GetCharFormat(void) 
{
    return (USBUART_1506_linesCoding[USBUART_1506_activeCom][USBUART_1506_LINE_CODING_STOP_BITS]);
}


/*******************************************************************************
* Function Name: USBUART_1506_GetParityType
****************************************************************************//**
*
...
{
    return (USBUART_1506_linesCoding[USBUART_1506_activeCom][USBUART_1506_LINE_CODING_PARITY]);
}


/***************************************************************************
* Function Name: USBUART_1506_GetDataBits
************************************************************************//**
*
...
{
    return (USBUART_1506_linesCoding[USBUART_1506_activeCom][USBUART_1506_LINE_CODING_DATA_BITS]);
}


/***************************************************************************
* Function Name: USBUART_1506_GetLineControl
************************************************************************//**
*
...
*
* \globalvars
*  USBUART_1506_linesControlBitmap: used to get a parameter.
*
*******************************************************************************/
uint16 USBUART_1506_GetLineControl(void) 
{
    return (USBUART_1506_linesControlBitmap[USBUART_1506_activeCom]);
}


/*******************************************************************************
* Function Name: USBUART_1506_SendSerialState
****************************************************************************//**
*
...
*  USBUART_1506_NotificationIsReady() API must be called to check if the
...
{
    uint8 epNumber = USBUART_1506_cdcCommInInterruptEp[USBUART_1506_activeCom];

    if(USBUART_1506_SERIAL_STATE_SIZE <= USBUART_1506_EP[epNumber].bufferSize)
    {
        /* Save current SERIAL_STATE bitmap. */
        USBUART_1506_serialStateBitmap[USBUART_1506_activeCom] = serialState;

        /* Add interface number */
        USBUART_1506_serialStateNotification.wIndex = USBUART_1506_EP[epNumber].interface;

        /*Form SERIAL_STATE data*/
        USBUART_1506_serialStateNotification.wSerialState =    LO8(USBUART_1506_serialStateBitmap[USBUART_1506_activeCom]);
        USBUART_1506_serialStateNotification.wSerialStateMSB = HI8(USBUART_1506_serialStateBitmap[USBUART_1506_activeCom]);

        USBUART_1506_LoadInEP(epNumber, (uint8 *) &USBUART_1506_serialStateNotification, sizeof(USBUART_1506_serialStateNotification));
    }
}


/*******************************************************************************
* Function Name: USBUART_1506_GetSerialState
****************************************************************************//**
*
...
{
    return USBUART_1506_serialStateBitmap[USBUART_1506_activeCom];
}


/*******************************************************************************
* Function Name: USBUART_1506_NotificationIsReady
****************************************************************************//**
*
...
*  USBUART_1506_SendSerialState() to ensure that any previous
...
{
    return (USBUART_1506_EP[USBUART_1506_cdcCommInInterruptEp[USBUART_1506_activeCom]].apiEpState);
}


/*******************************************************************************
* Function Name: USBUART_1506_SetComPort
****************************************************************************//**
*
...
{
    if ((USBUART_1506_activeCom != comNumber) && \
            (comNumber < USBUART_1506_MAX_MULTI_COM_NUM ))
    {
        USBUART_1506_activeCom = comNumber;
    }
}


/*******************************************************************************
* Function Name: USBUART_1506_GetComPort
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: USBUART_1506_Cdc_EpInit
************************************************************************//**
*
*  \internal
...
*   USBUART_1506_cdcDataInEp[], USBUART_1506_cdcCommInInterruptEp[],
*   USBUART_1506_cdcDataOutEp[]
*
...
uint8 USBUART_1506_Cdc_EpInit(const T_USBUART_1506_EP_SETTINGS_BLOCK CYCODE *pEP, uint8 epNum, uint8 cdcComNums) 
{
    ...
    {
        if (epType != USBUART_1506_EP_TYPE_INT)
        {
            USBUART_1506_cdcDataInEp[USBUART_1506_GET_EP_COM_NUM(cdcComNums, USBUART_1506_CDC_IN_EP)] = epNum;
            cdcComNums |= (uint8)(USBUART_1506_COM_PORT2 << USBUART_1506_CDC_IN_EP);
        }
        else
        {

            USBUART_1506_cdcCommInInterruptEp[USBUART_1506_GET_EP_COM_NUM(cdcComNums, USBUART_1506_CDC_NOTE_EP)] = epNum;
            cdcComNums |= (uint8)(USBUART_1506_COM_PORT2 << USBUART_1506_CDC_NOTE_EP);
        }
    }
    else
    {
        if (epType != USBUART_1506_EP_TYPE_INT)
        {
            USBUART_1506_cdcDataOutEp[USBUART_1506_GET_EP_COM_NUM(cdcComNums, USBUART_1506_CDC_OUT_EP)] = epNum;
            cdcComNums |= (uint8)(USBUART_1506_COM_PORT2 << USBUART_1506_CDC_OUT_EP);
        }
    }
    return (cdcComNums);
}
